...
Lambda Extension API so non-Tarn local lambdas can fetch secrets through
localhost:2773.`,
		Example: `  tarn secrets proxy
  tarn secrets proxy --port 2773 --session-token local-dev-token
  tarn secrets proxy --upstream http://localhost:4566`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Fprintf(cmd.OutOrStdout(), "Upstream Tarn endpoint: %s\n", upstream)
	...
	cmd.Flags().StringVar(&upstream, "upstream", "", "Upstream Tarn endpoint (default derived from TARN_ENDPOINT or root host/port)")
