...
export function collectionVariables(
  ...
  vars.set("invokeBase", {
    key: "invokeBase",
    value: normalizeInvokeUrl(gw.invokeUrl),
    type: "string",
  });
...
function buildExampleResponse(
  ...
  const isJson =
    ex.headers?.["Content-Type"]?.includes("json") ?? ex.body?.trimStart().startsWith("{") ?? false;
  return {
...
function buildChaosExamples(round: ChaosRound, request: Record<string, unknown>): unknown[] {
  ...
  return sources.map((ex, i) =>
    buildExampleResponse(ex, `round ${i + 1} of ${sources.length}`, request),
  );
}

export function collectionItems(
  gw: GatewaySummary,
  chaosResults?: Map<string, ChaosRound>,
): unknown[] {
  const details = gw.routeDetails;
  if (details && details.length > 0) {
    return details.map((detail) => {
      ...
      const exportedMethod =
        method === "ANY" || method === "$default" ? "GET" : method.toUpperCase();
      const rawPath =
        method === "$default"
      ...
      if (method === "ANY")
        description = `Generated from ${detail.routeKey}. Exported as GET placeholder — change method as needed.`;
      else if (method === "$default")
        description = "Generated from $default route. Set {{defaultPath}} before sending.";
...
export function buildPostmanEnvironment(gw: GatewaySummary) {
  return {
    ...
    values: collectionVariables(gw).map((v) => ({
      key: v.key,
      value: v.value,
      enabled: true,
      type: v.type ?? "text",
