...
>
	{#each functions as fn}
		<TableRow>
			...
			<TableCell><Badge variant={stateBadgeVariant(fn.state)}>{fn.state}</Badge></TableCell>
			<TableCell class="font-mono text-text-muted">{fn.messagesProcessed}</TableCell>
			<TableCell class="font-mono text-text-muted">{fn.memoryMB} MB</TableCell>
			<TableCell class="font-mono text-text-muted">{fn.timeoutSec}s</TableCell>
			<TableCell class="font-mono text-text-muted">{formatBytes(fn.codeSize)}</TableCell>
			<TableCell class="text-text-muted">{fn.layers}</TableCell>
			<TableCell class="text-text-muted">{fn.tagCount}</TableCell>
			<TableCell class="text-text-faint text-xs">{formatDate(fn.lastModified)}</TableCell>
		</TableRow>
	{/each}
</ResourceTable>
