<script lang="ts">
	...
	import type { GatewaySummary, RouteDetail, ChaosRound, ScanSourceResponse, ScanMatch, ProbeBody } from '$lib/types';
	import { normalizeInvokeUrl, buildPostmanCollection, downloadJSON, slugify } from '$lib/postman';

	let { gateways }: { gateways: GatewaySummary[] } = $props();

	// Only gateways with routeDetails can be chaos-probed
	const probeableGateways = $derived(gateways.filter((gw) => gw.routeDetails?.length));
	...
	const successCount = $derived(
		[...results.values()].filter((r) => (r.statusCode ?? 0) >= 200 && (r.statusCode ?? 0) < 300).length
	);
	...
	function statusColor(code: number | undefined) {
		if (!code) return 'text-text-faint';
	...
	function methodVariant(m: string): 'default' | 'secondary' | 'amber' | 'outline' | 'destructive' {
	...
	function routeProbeBodies(detail: RouteDetail): ProbeBody[] | undefined {
		...
		if (matchMap.has(fn)) {
			...
			for (const [k, v] of matchMap) {
				if (fn.includes(k) || k.includes(fn)) { match = v; break; }
			}
		}
	...
	async function scanSource() {
		if (!sourceDir.trim() || scanning) return;
		scanning = true;
		scanError = '';
		scanResult = null;
		...
		try {
			const resp = await fetch('/_openstack/admin/chaos/source', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ baseDir: sourceDir.trim(), functionNames: unique })
			});
			if (!resp.ok) {
	...
	async function reprobeRoute(gw: GatewaySummary, detail: RouteDetail, id: string) {
		...
		if (lastExample?.requestBody) {
			try { baseBody = JSON.parse(lastExample.requestBody); } catch { /* ignore */ }
		}
...
<div class="space-y-4">
	...
	<div class="flex items-center justify-between gap-4">
		<div>
			<h2 class="text-sm font-semibold text-text flex items-center gap-2">
				<LightningIcon size={14} class="text-accent" />
				Chaos Probe
			</h2>
			<p class="mt-0.5 text-[11px] text-text-faint">
				Exhaust all validation layers per route — captures every 400 and the final 200 as named
				Postman examples.
			</p>
		</div>
		...
		<div class="flex items-center gap-2 shrink-0">
			{#if probing}
				<button
					type="button"
					onclick={stop}
					class="inline-flex items-center gap-1.5 rounded-md border border-red-500/40 bg-red-500/10 px-3 py-1.5 text-xs text-red-400 hover:bg-red-500/20 transition-colors"
				>
					<StopIcon size={12} />
					Stop
				</button>
			{:else}
				<button
					type="button"
					onclick={() => setAll(true)}
					class="text-[11px] text-text-faint hover:text-text transition-colors"
				>
					Select all
				</button>
				<button
				...
				<button
					...
					class="inline-flex items-center gap-1.5 rounded-md border border-accent-strong bg-accent-muted px-3 py-1.5 text-xs text-accent hover:bg-accent/20 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
				>
					<PlayIcon size={12} />
	...
	<div class="rounded-lg border border-border bg-bg-raised overflow-hidden">
		<div class="flex items-center gap-2 border-b border-border px-3 py-2 bg-bg-overlay/40">
			<FolderOpenIcon size={13} class="text-text-faint shrink-0" />
			<span class="text-[11px] font-semibold text-text">Schema Source</span>
			<span class="ml-1 text-[10px] text-text-faint">— provide a local Lambda repo directory to generate schema-driven probes</span>
		</div>
		<div class="px-3 py-2.5 space-y-2">
			<div class="flex items-center gap-2">
				<input
					...
					class="flex-1 min-w-0 rounded border border-border bg-bg px-2.5 py-1.5 font-mono text-[11px] text-text placeholder:text-text-faint/40 focus:border-accent/60 focus:outline-none"
				/>
				<button
					type="button"
					onclick={scanSource}
					disabled={!sourceDir.trim() || scanning}
					class="inline-flex items-center gap-1.5 rounded border border-border bg-bg-raised px-3 py-1.5 text-[11px] text-text hover:bg-bg-overlay transition-colors disabled:opacity-40 disabled:cursor-not-allowed shrink-0"
				>
					{#if scanning}
			...
			{#if scanError}
				<div class="flex items-center gap-1.5 font-mono text-[10px] text-red-400">
			...
			{#if scanResult}
				...
				{@const withSchema = scanResult.matches.filter((m) => m.schemasTs).length}
				...
				<div class="flex items-center gap-3 font-mono text-[10px]">
					...
					{#if withSchema > 0}
						<span class="text-accent">{withSchema} with schema</span>
					{/if}
					{#if unmatched > 0}
						<span class="text-text-faint">{unmatched} unmatched</span>
					{/if}
				</div>
				...
				{#if scanResult.matches.length > 0}
					<div class="space-y-1 max-h-40 overflow-y-auto">
						{#each scanResult.matches as m (m.functionName)}
							<div class="flex items-start gap-2 rounded border border-border/50 bg-bg px-2.5 py-1.5 text-[10px]">
								<div class="flex-1 min-w-0">
									<div class="flex items-center gap-2">
										<span class="font-mono text-text truncate">{m.functionName}</span>
										<span class="font-mono text-[9px] text-text-faint/60">
											{Math.round(m.score * 100)}% match
										</span>
									</div>
									<div class="flex items-center gap-2 mt-0.5">
										<span class="font-mono text-[9px] text-text-faint truncate">{m.dir}</span>
									</div>
								</div>
								<div class="shrink-0 flex gap-1.5">
									{#if m.schemasTs}
										<span class="rounded border border-accent-strong bg-accent-muted px-1 py-0.5 font-mono text-[9px] text-accent">
											schema
										</span>
									{/if}
									{#if m.eventFiles?.length}
										<span class="rounded border border-border bg-bg-raised px-1 py-0.5 font-mono text-[9px] text-text-faint">
				...
				{#if scanResult.unmatched.length > 0}
					<div class="font-mono text-[10px] text-text-faint/60">
	...
	{#if probeableGateways.length === 0}
		<div class="rounded-lg border border-border bg-bg-raised px-4 py-8 text-center">
			<p class="text-sm text-text-faint">No gateways with route details available.</p>
			<p class="mt-1 text-[11px] text-text-faint">Refresh the dashboard after deploying your API Gateway.</p>
		</div>
	{:else}
		<div class="space-y-3">
			{#each probeableGateways as gw (gw.apiId)}
				...
				{@const gwDone = results.size > 0 && gwIds.every((id) => results.has(id))}

				<div class="rounded-lg border border-border bg-bg-raised overflow-hidden">
					<!-- Gateway header -->
					<div class="flex items-center justify-between gap-3 border-b border-border px-3 py-2 bg-bg-overlay/40">
						<div class="flex items-center gap-2 min-w-0">
							<button
								type="button"
								onclick={() => toggleGateway(gw)}
								class="shrink-0 text-text-muted hover:text-text transition-colors"
								aria-label={allChecked ? 'Deselect all routes' : 'Select all routes'}
							>
								{#if allChecked}
									<CheckSquareIcon size={14} class="text-accent" />
								{:else}
									<SquareIcon size={14} />
								{/if}
							</button>
							<span class="text-xs font-semibold text-text truncate">{gw.name}</span>
							<Badge variant="secondary" class="shrink-0 text-[10px]">{gw.protocolType}</Badge>
							<Badge variant="outline" class="shrink-0 text-[10px] px-1 py-0 font-mono">{gw.version}</Badge>
							<span class="hidden sm:block font-mono text-[10px] text-text-faint truncate">
						...
						{#if gwDone}
							<button
								...
								class="inline-flex shrink-0 items-center gap-1 rounded border border-accent-strong bg-accent-muted px-2 py-1 text-[11px] text-accent hover:bg-accent/20 transition-colors"
							>
							...
							<div>
								...
								<div class="flex items-center gap-2 px-3 py-2 text-[11px]">
									...
									<button
										...
										disabled={probing}
										class="shrink-0 text-text-muted hover:text-text transition-colors disabled:opacity-40"
										aria-label={selected.has(id) ? 'Deselect' : 'Select'}
									>
										{#if selected.has(id)}
											<CheckSquareIcon size={13} class="text-accent" />
										{:else}
											<SquareIcon size={13} />
										{/if}
									</button>
									...
									<!-- Method -->
									<Badge variant={methodVariant(detail.method ?? 'GET')} class="shrink-0 text-[10px] font-mono">
										{detail.method ?? 'GET'}
									</Badge>

									<!-- Path -->
									<span class="flex-1 truncate font-mono text-text-muted">
										{detail.path ?? detail.routeKey}
									</span>

									<!-- Status -->
									{#if isProbing}
										<span class="flex items-center gap-1 text-accent shrink-0">
											<SpinnerGapIcon size={10} class="animate-spin" />
											probing
										</span>
									{:else if round}
										{@const codes = round.examples?.map((e) => e.statusCode) ?? [round.statusCode]}
										<span class="font-mono {statusColor(round.statusCode)} shrink-0">
											{round.statusCode}
										</span>
										{#if codes.length > 1}
											<span class="font-mono text-text-faint/60 text-[10px] shrink-0">
												{codes.join('→')}
											</span>
										{/if}
										<span class="text-text-faint shrink-0">{round.durationMs}ms</span>
										{#if round.examples?.length}
											<button
												type="button"
												onclick={() => toggleExpand(id)}
												class="flex items-center gap-0.5 text-text-faint hover:text-text transition-colors shrink-0"
												aria-label={isExpanded ? 'Collapse examples' : 'Expand examples'}
											>
												{#if isExpanded}
												...
												{/if}
												<span class="text-[10px]">{round.examples.length} example{round.examples.length !== 1 ? 's' : ''}</span>
											</button>
										{/if}
									{:else if probing && selected.has(id)}
										<span class="text-text-faint/50 shrink-0">queued</span>
									{:else if selected.has(id)}
										<span class="text-text-faint/40 shrink-0">pending</span>
									{/if}
								</div>
								...
								{#if isExpanded && round?.examples?.length}
									...
									<div class="border-t border-border/40 bg-bg divide-y divide-border/30">
										{#each round.examples as ex, i (i)}
											...
											{@const hasReqBody = ex.requestBody !== undefined && ex.requestBody !== null}
											{@const hasResBody = !!ex.body}
											{@const canFormat = hasResBody && isJson(ex.body!)}

											<div>
												<!-- Example header row -->
												<div class="flex items-center gap-2 px-3 py-1.5 font-mono text-[10px]">
													<span class="text-text-faint/60 shrink-0 max-w-[7rem] truncate" title={roundLabel(i, total, ex.label)}>{roundLabel(i, total, ex.label)}</span>
													<span class="{statusColor(ex.statusCode)} font-semibold shrink-0">{ex.statusCode}</span>
													<span class="text-text-faint shrink-0">{ex.durationMs}ms</span>
													<span class="flex-1"></span>
													<button
														type="button"
														onclick={() => toggleExReq(id, i)}
														class="rounded px-1.5 py-0.5 text-[9px] transition-colors shrink-0
															{showReq ? 'bg-bg-overlay text-text border border-border' : 'text-text-faint/60 hover:text-text'}"
														title="Show request inputs"
													>req</button>
													{#if canFormat}
														<button
															type="button"
															...
															class="rounded px-1.5 py-0.5 text-[9px] font-sans transition-colors shrink-0
																{formatted ? 'bg-bg-overlay text-text border border-border' : 'text-text-faint/60 hover:text-text'}"
												...
												{#if showReq}
													<div class="mx-3 mb-2 rounded border border-border/50 bg-bg-overlay/40 overflow-hidden">
														<div class="px-2.5 py-1.5 space-y-2">
															<div>
																<p class="mb-1 font-mono text-[9px] uppercase tracking-widest text-text-faint/50">headers sent</p>
																{#if reqHeaders.length}
																	<div class="space-y-0.5">
																		{#each reqHeaders as [k, v] (k)}
																			<div class="flex gap-2 font-mono text-[10px]">
																				<span class="text-text-faint shrink-0">{k}</span>
																				<span class="text-accent/80 break-all">{v}</span>
																			</div>
																		{/each}
																	</div>
																{:else}
																	<p class="font-mono text-[10px] text-text-faint/40 italic">no custom headers</p>
																{/if}
															</div>
															{#if hasReqBody}
																<div>
																	<p class="mb-1 font-mono text-[9px] uppercase tracking-widest text-text-faint/50">body sent</p>
																	<pre class="font-mono text-[10px] text-text-muted whitespace-pre-wrap break-all leading-relaxed">{tryFormat(ex.requestBody ?? '')}</pre>
																</div>
															{/if}
														</div>
													</div>
												...
												{#if hasResBody}
													<div class="px-3 pb-2.5">
														<p class="mb-1 font-mono text-[9px] uppercase tracking-widest text-text-faint/50">response</p>
														<pre class="font-mono text-[10px] text-text-muted whitespace-pre-wrap break-all leading-relaxed max-h-56 overflow-y-auto rounded">{formatted ? tryFormat(ex.body!) : truncate(ex.body!, 800)}</pre>
													</div>
												{/if}
											</div>
										{/each}
								...
								{#if round?.needsInput && !reProbing.has(id)}
									<div class="border-t border-amber-500/20 bg-amber-500/5 px-3 py-2.5">
										<p class="mb-2 font-mono text-[10px] text-amber-400">
											Enum field{round.stuckFields && round.stuckFields.length !== 1 ? 's' : ''} need a valid value to continue.
										</p>
										<div class="space-y-1.5 mb-2.5">
											{#each round.stuckFields ?? [] as field (field)}
												<div class="flex items-center gap-2">
													<span class="font-mono text-[10px] text-text-faint w-28 shrink-0 truncate">{field}</span>
													<div class="flex-1 min-w-0 space-y-0.5">
														<input
															type="text"
															placeholder="value"
															value={getOverride(id, field)}
															oninput={(e) => setOverride(id, field, (e.target as HTMLInputElement).value)}
															class="w-full rounded border border-border bg-bg px-2 py-1 font-mono text-[10px] text-text placeholder:text-text-faint/40 focus:border-amber-500/60 focus:outline-none"
														/>
														{#if round.stuckOptions?.[field]?.length}
															<p class="font-mono text-[9px] text-text-faint/50">
								...
								{:else if reProbing.has(id)}
									<div class="border-t border-border/30 bg-bg px-3 py-2 flex items-center gap-1.5 font-mono text-[10px] text-accent">
										<SpinnerGapIcon size={10} class="animate-spin" />
										re-probing…
									</div>
								{/if}
		...
		{#if probing}
			<div class="relative h-7 rounded border border-border overflow-hidden bg-bg">
				<div
					class="absolute inset-y-0 left-0 bg-accent/15 transition-all duration-300"
					style="width: {totalSelected > 0 ? Math.round((results.size / totalSelected) * 100) : 0}%"
				></div>
		...
		{#if !probing && results.size > 0}
			<div class="flex rounded border border-border overflow-hidden text-xs">
				<!-- left: stats fill -->
				<div class="flex flex-1 items-center gap-4 bg-bg-raised px-4 py-2.5 min-w-0">
					<span class="font-mono text-text-faint shrink-0">
						{results.size} route{results.size !== 1 ? 's' : ''}
					</span>
					<span class="text-border select-none">|</span>
					<span class="font-mono shrink-0" class:text-green-500={successCount > 0} class:text-text-faint={successCount === 0}>
					...
					<span class="hidden sm:block font-mono text-[10px] text-text-faint/60 truncate">
						{probeableGateways.map((g) => g.name).join(' · ')}
					</span>
				</div>
				<!-- right: action buttons -->
				<div class="flex shrink-0 divide-x divide-border border-l border-border">
					<button
						type="button"
						onclick={run}
						class="flex items-center gap-1.5 px-3 py-2.5 text-text-faint hover:bg-bg-overlay hover:text-text transition-colors"
						title="Re-run probes"
					>
						<PlayIcon size={11} />
					...
					<button
						type="button"
						onclick={downloadAll}
						class="flex items-center gap-1.5 px-4 py-2.5 bg-accent-muted text-accent hover:bg-accent/20 transition-colors font-medium"
					>
