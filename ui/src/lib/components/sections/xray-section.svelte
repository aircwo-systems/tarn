<script lang="ts">
		...
		searchQuery.trim()
			? traces.filter((t) => {
					...
					return (
						...
						t.spans.some((s) => s.name.toLowerCase().includes(q) || s.kind.toLowerCase().includes(q))
					);
				})
			: traces
	);
	...
	const selectedTrace = $derived(filteredTraces.find((t) => t.id === effectiveId) ?? null);

	const errorCount = $derived(traces.filter((t) => t.status >= 500).length);
	const clientErrorCount = $derived(traces.filter((t) => t.status >= 400 && t.status < 500).length);
	const avgMs = $derived(
		traces.length > 0
	...
	function statusBadgeClass(s: number): string {
		if (s >= 500) return 'text-red border-red/40 bg-red/8';
		if (s >= 400) return 'text-amber border-amber/40 bg-amber/8';
		return 'text-accent border-accent-strong bg-accent-muted';
	}

	function spanBadge(span: TraceSpan): { label: string; colorClass: string } {
		if (span.status === 'error') return { label: '✕ error', colorClass: 'text-red' };
		if (span.status === 'client_error') return { label: '⚠ warn', colorClass: 'text-amber' };
		return { label: '✓ ok', colorClass: 'text-accent' };
	}
...
<div class="space-y-4">
	<!-- Header strip -->
	<div
		class="flex items-center justify-between gap-4 flex-wrap rounded-lg border border-border bg-bg-raised px-4 py-3"
	>
		<div class="flex items-center gap-3">
			<div class="flex items-center justify-center h-8 w-8 rounded-md bg-accent/10">
				<DetectiveIcon size={16} class="text-accent" />
			</div>
			<div>
				<h2 class="text-sm font-semibold text-text">X-Ray Traces</h2>
				<p class="text-[10px] font-mono text-text-faint">End-to-end request flow visualiser</p>
			</div>
		</div>
		<div class="flex items-center gap-4 text-[11px] font-mono">
			<span class="text-text-faint">{traces.length} trace{traces.length !== 1 ? 's' : ''}</span>
			{#if errorCount > 0}
				<span class="text-red">{errorCount} 5xx</span>
			{/if}
			{#if clientErrorCount > 0}
				<span class="text-amber">{clientErrorCount} 4xx</span>
			{/if}
			{#if traces.length > 0}
				<span class="text-text-faint">avg {formatMs(avgMs)}</span>
			{/if}
			{#if p95Ms > 0}
				<span class="text-text-faint">p95 {formatMs(p95Ms)}</span>
			{/if}
		</div>
	</div>
	...
	{#if traces.length === 0}
		<!-- Empty state -->
		<div
			class="rounded-lg border border-border bg-bg-raised flex flex-col items-center gap-4 px-8 py-16"
		>
			<div
				class="flex items-center justify-center h-12 w-12 rounded-xl border border-border bg-bg-surface"
			>
				<DetectiveIcon size={24} class="text-text-faint" />
			</div>
			<div class="text-center space-y-1.5">
				<p class="text-sm font-semibold text-text-muted">No traces recorded yet</p>
				<p class="text-xs text-text-faint max-w-sm leading-relaxed">
					Make HTTP requests through an API Gateway or trigger SQS event source mappings. Traces
					will appear here showing the full request flow through each component.
				</p>
			</div>
			<div class="flex items-center gap-5 mt-1 text-[10px] font-mono text-text-faint">
				<span class="flex items-center gap-1.5">
					<span class="h-2 w-2 rounded-sm inline-block" style="background:var(--color-red);opacity:0.6"></span>
					API Gateway
				</span>
				<span class="flex items-center gap-1.5">
					<span class="h-2 w-2 rounded-sm inline-block" style="background:var(--color-accent);opacity:0.6"></span>
					Lambda
				</span>
				<span class="flex items-center gap-1.5">
					<span class="h-2 w-2 rounded-sm inline-block" style="background:var(--color-amber);opacity:0.6"></span>
					SQS / DLQ
				</span>
				<span class="flex items-center gap-1.5">
					<span class="h-2 w-2 rounded-sm inline-block" style="background:var(--color-blue);opacity:0.6"></span>
					Secrets / DB
				</span>
			</div>
		</div>
	{:else}
		<div class="flex gap-4 items-start">
			<!-- ─── Trace list panel ─── -->
			<div
				class="w-72 shrink-0 rounded-lg border border-border bg-bg-raised overflow-hidden flex flex-col"
			>
				<div class="px-2.5 py-2 border-b border-border">
					<input
						...
						class="w-full rounded border border-border bg-bg-surface px-2.5 py-1.5 text-xs text-text placeholder:text-text-faint outline-none focus:ring-1 focus:ring-accent"
					/>
				</div>
			...
			{#if selectedTrace}
				<div class="flex-1 min-w-0 space-y-3">
					...
					<div class="rounded-lg border border-border bg-bg-raised px-4 py-3">
						<div class="flex items-center gap-3 flex-wrap min-w-0">
							{#if selectedTrace.method}
								<span
									class="shrink-0 font-mono text-[11px] px-1.5 py-0.5 rounded border border-border text-text-muted"
									>{selectedTrace.method}</span
								>
							{/if}
							<span class="font-mono text-sm text-text truncate flex-1 min-w-0">
								{selectedTrace.path ??
							...
							<span
								class="shrink-0 inline-flex items-center rounded-full border px-2.5 py-0.5 text-[11px] font-mono {statusBadgeClass(selectedTrace.status)}"
								>{selectedTrace.status}</span
							>
							<span class="shrink-0 text-[11px] font-mono text-text-faint"
						...
						{#if selectedTrace.spans.length > 0}
							<div
								class="mt-2 text-[10px] font-mono text-text-faint flex items-center gap-1 flex-wrap"
							>
								{#each selectedTrace.spans as span, i (i)}
									{#if i > 0}
										<span class="opacity-40 select-none">→</span>
									{/if}
									<span style="color: {spanColor(span.kind)}">{spanKindLabel(span.kind)}</span>
								{/each}
							</div>
						{/if}
					</div>
					...
					{#if selectedTrace.spans.length > 0}
						...
						{@const cw = flowCW(n)}
						{@const rows = buildWaterfall(selectedTrace.spans, selectedTrace.durationMs)}
						...
						<div class="rounded-lg border border-border bg-bg-raised px-4 py-3">
								...
								{#each rows as { span, offsetPct, widthPct }, i (i)}
									<div class="flex items-center gap-2.5">
										...
										<span
											...
											style="color:{spanColor(span.kind)};background:{spanColor(span.kind)}18;border:1px solid {spanColor(span.kind)}30"
										>
											{spanKindLabel(span.kind)}
										</span>
										<!-- Name -->
										<span
											class="w-28 text-[11px] font-mono text-text-muted truncate shrink-0"
											>{span.name}</span
										>
										<!-- Bar track -->
										...
										<!-- Duration -->
										<span class="w-12 text-right text-[10px] font-mono text-text-faint shrink-0"
							...
							<!-- Ruler -->
							<div
								class="flex items-center justify-between mt-2 pt-1.5 border-t border-border text-[10px] font-mono text-text-faint"
							>
								<span>0ms</span>
								...
								{#each selectedTrace.spans as span, i (i)}
									...
									>
										...
										<div
											...
											style="color:{spanColor(span.kind)};background:{spanColor(span.kind)}18;border:1px solid {spanColor(span.kind)}35"
										>
											{i + 1}
										</div>
										<div class="flex-1 min-w-0">
											<div class="flex items-center gap-2 flex-wrap mb-0.5">
												<span class="text-[11px] font-semibold text-text truncate"
													>{span.name}</span
												>
												<span
					...
					{:else}
						<div class="rounded-lg border border-border bg-bg-raised px-4 py-10 text-center">
							<p class="text-xs text-text-faint">No span data available for this trace</p>
						</div>
					{/if}
				</div>
			{:else if filteredTraces.length === 0 && searchQuery}
				<div class="flex-1 rounded-lg border border-border bg-bg-raised px-4 py-12 text-center">
					<p class="text-xs text-text-faint">No traces match "{searchQuery}"</p>
				</div>
			{/if}
		</div>
	{/if}
