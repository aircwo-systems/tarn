<script lang="ts">
  ...
  function onKeydown(e: KeyboardEvent) {
    ...
    e.preventDefault();
    const idx = visible.findIndex((topic) => topic.name === selectedName);
...
<div class="topic-list" onkeydown={onKeydown}>
  <label class="search">
    <MagnifyingGlassIcon size={12} />
    <input placeholder="Filter topics" bind:value={query} aria-label="Filter topics" />
    <span class="count">{visible.length}</span>
  </label>

  <div class="rows">
    {#each visible as topic (topic.name)}
      <RcListRow
        mono
        title={topic.name}
        sub="{topic.fifo ? 'FIFO' : 'Standard'} · {topic.subscriptions} subscription{topic.subscriptions === 1 ? '' : 's'}"
        selected={topic.name === selectedName}
        onclick={() => onselect(topic.name)}
      >
        {#snippet trailing()}
          {#if topic.subscriptions > 0}
            <span class="calls" title="{topic.subscriptions} subscriptions">{topic.subscriptions}</span>
          {/if}
        {/snippet}
      </RcListRow>
    {:else}
...
<style>
  .topic-list { display: flex; flex-direction: column; gap: 8px; min-height: 0; }
  .search {
